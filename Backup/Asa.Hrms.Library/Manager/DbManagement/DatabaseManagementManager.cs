...
using System.Xml;
using Asa.Hrms.WebSite.Code.Utility;
using Asa.Hrms.Data;
using System.Collections.Specialized;
using Asa.Hrms.Library.Properties;

namespace Asa.Hrms.WebSite.Code.Manager.DbManagement
{
    public class DatabaseManagementManager
    {
        ...
        {
            return this.dbDefault.GetDataSet("SELECT name FROM sys.databases WHERE name = N'" + Asa.Hrms.Utility.Configuration.DatabaseName + "'").Tables[0].Rows.Count > 0;
        }
        ...
        public Message Backup(String file)
        {
            ...
            try
            {
                msg = this.SetAccessMode(Asa.Hrms.Utility.Configuration.DatabaseName, ACCESS_MODE_SINGLE_USER);
                ...
                bu.Database = Asa.Hrms.Utility.Configuration.DatabaseName;
                bu.Devices.Add(bdi);
                bu.Checksum = true;
                bu.Initialize = true;
                ...
                msg = this.SetAccessMode(Asa.Hrms.Utility.Configuration.DatabaseName, ACCESS_MODE_MULTI_USER);
        ...
        public Message Restore(string backupFileName)
        {
            ...
            try
            {
                ...
                res.Database = Asa.Hrms.Utility.Configuration.DatabaseName;
        ...
        private Message CreateDatabase(bool drop, out bool fresh)
        {
            ...
            if (Asa.Hrms.Utility.Configuration.Provider == "SQL Server")
            {
                try
                {
                    //try
                    ...
                    if (this.dbDefault.GetDataSet("SELECT name FROM sys.databases WHERE name = N'" + Asa.Hrms.Utility.Configuration.DatabaseName + "'").Tables[0].Rows.Count > 0)
                    {
                        if (drop)
                        {
                            msg = this.SetAccessMode(Asa.Hrms.Utility.Configuration.DatabaseName, ACCESS_MODE_SINGLE_USER);
                            ...
                            if (this.dbDefault.ExecuteUpdate("DROP DATABASE [" + Asa.Hrms.Utility.Configuration.DatabaseName + "]") == 0)
                            {
                                msg.Type = MessageType.Error;
                                msg.Msg = "Unable to drop " + Asa.Hrms.Utility.Configuration.DatabaseName + " database";
                                return msg;
                            }
                            else
                            {
                    ...
                    if (create)
                    {
                        ...
                        if (this.dbDefault.ExecuteUpdate("CREATE DATABASE [" + Asa.Hrms.Utility.Configuration.DatabaseName + "]") == 0)
                        {
                            msg.Type = MessageType.Error;
                            msg.Msg = "Unable to create" + Asa.Hrms.Utility.Configuration.DatabaseName + " database";
                            return msg;
                        }
                        ...
                        if (this.dbDefault.ExecuteUpdate("ALTER DATABASE [" + Asa.Hrms.Utility.Configuration.DatabaseName + "] SET RECOVERY FULL") == 0)
                        {
                            msg.Type = MessageType.Error;
                            msg.Msg = "Unable to set recovery mode for " + Asa.Hrms.Utility.Configuration.DatabaseName + " database";
                            return msg;
                        }

                        if (this.dbDefault.ExecuteUpdate("ALTER DATABASE [" + Asa.Hrms.Utility.Configuration.DatabaseName + "] SET AUTO_SHRINK ON") == 0)
                        {
                            msg.Type = MessageType.Error;
                            msg.Msg = "Unable to set auto shrink on for " + Asa.Hrms.Utility.Configuration.DatabaseName + " database";
                            return msg;
                        }
                    }
                }
        ...
        public DatabaseConfig LoadDatabaseConfig(String xml)
        {
            ...
            for (int i = 0; i < xmlNode.Count; i++)
            {
                if (xmlNode[i].Attributes != null)
                {
                    ...
                    {
                        if (xmlNode[i].SelectSingleNode("script[(@provider='" + Asa.Hrms.Utility.Configuration.StandardProvider + "' or @provider='" + Asa.Hrms.Utility.Configuration.Provider + "')]") != null)
                        {
                            dc.Script = xmlNode[i].SelectSingleNode("script[(@provider='" + Asa.Hrms.Utility.Configuration.StandardProvider + "' or @provider='" + Asa.Hrms.Utility.Configuration.Provider + "')]").InnerText.Trim();
                        }
                    }
